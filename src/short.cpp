...
int print_short_fft_common_factor(int N, std::vector<std::string> in_, std::vector<std::string> out_, bool real = false) {
	int ocnt = 0;
	...
	if (!real) {
		...
		if (N > R) {
			for (int r = 0; r < R; r++) {
				print("{\n");
				indent();
				ocnt += print_short_fft_common_factor(N / R, in1[r], out1[r]);
				deindent();
				print("}\n");
			}
		}
...
int print_short_fft_singleton(int N, std::vector<std::string> in_, std::vector<std::string> out_, bool real = false) {
	int ocnt = 0;
	std::vector<int> ri(N);
	std::vector<int> ii(N);
	if (real) {
...
int print_short_fft_split_radix(int N, std::vector<std::string> in_, std::vector<std::string> out_, bool real = false) {
	int ocnt = 0;
	std::vector<const char*> in(in_.size());
	std::vector<const char*> out(out_.size());
	for (int i = 0; i < in.size(); i++) {
	...
	print_vars(vars);
	print("/* begin split radix short transform - %s - length %i*/\n", real ? "real" : "complex", N);
	if (real) {
		int R = 4;
		std::vector<std::vector<std::string>> in1(R);
		std::vector<std::vector<std::string>> out1(R);
		for (int n = 0; n < N / R; n++) {
			for (int r = 0; r < R; r++) {
				int r0 = r == 2 ? 0 : r;
				in1[r0].push_back(in[R * n + r]);
				if (n < N / R / 2 + 1) {
					out1[r0].push_back(std::string("y") + std::to_string(N) + "r" + std::to_string(R * n + r));
				} else {
					out1[r0].push_back(std::string("y") + std::to_string(N) + "i" + std::to_string(R * (N / R - n) + r));
				}
			}
		}
		printf("\n");
		if (N > 4) {
			print("{\n");
			indent();
			ocnt += print_real_short_fft(N / 4, in1[1], out1[1]);
			deindent();
			print("}\n");
			print("{\n");
			indent();
			ocnt += print_real_short_fft(N / 4, in1[3], out1[3]);
			deindent();
			print("}\n");
		} else {
			print("%s = %s;\n", out1[1][0].c_str(), in1[1][0].c_str());
			print("%s = %s;\n", out1[3][0].c_str(), in1[3][0].c_str());
		}
		print("{\n");
		indent();
		ocnt += print_real_short_fft(N / 2, in1[0], out1[0]);

		deindent();
		print("}\n");
		for (int k = 1; k < R; k++) {
			for (int n = 1; n <= ((N / R) - 1) / 2; n += 2) {
				const auto tw = std::polar(1.0, -2.0 * M_PI * n * k / N);
				int q = R * n + k;
				if (n * k == 0) {
		...
		}
		{
			...
			print("const auto zrs = (%s + %s);\n", in2[1].c_str(), in2[3].c_str());
			print("const auto zrd = (%s - %s);\n", in2[1].c_str(), in2[3].c_str());
			print("%s = %s + zrs;\n", out2[0].c_str(), in2[0].c_str());
			print("%s = %s;\n", out2[1].c_str(), in2[2].c_str());
			print("%s = -zrd;\n", out2[3].c_str());
			print("%s = %s - zrs;\n", out2[2].c_str(), in2[0].c_str());
			deindent();
			print("}\n");
		}
		std::vector<std::string> negs;
		for (int k = 1; k <= ((N / R) - 1) / 2; k++) {
			...
			print("const auto zrs = (%s + %s);\n", in2[2].c_str(), in2[6].c_str());
			print("const auto zis = (%s + %s);\n", in2[3].c_str(), in2[7].c_str());
			print("const auto zrd = (%s - %s);\n", in2[2].c_str(), in2[6].c_str());
			print("const auto zid = (%s - %s);\n", in2[3].c_str(), in2[7].c_str());
			print("%s = %s + zrs;\n", out2[0].c_str(), in2[0].c_str());
			print("%s = %s + zis;\n", out2[1].c_str(), in2[1].c_str());
			print("%s = %s + zid;\n", out2[2].c_str(), in2[4].c_str());
			print("%s = %s - zrd;\n", out2[3].c_str(), in2[5].c_str());
			print("%s = %s - zrs;\n", out2[4].c_str(), in2[0].c_str());
			print("%s = %s - zis;\n", out2[5].c_str(), in2[1].c_str());
			print("%s = %s - zid;\n", out2[6].c_str(), in2[4].c_str());
			print("%s = %s + zrd;\n", out2[7].c_str(), in2[5].c_str());
			deindent();
			print("}\n");
		}
		if ((N / R) % 2 == 0) {
			...
			print("const auto t1 = (%s - %s) * %.17e;\n", in2[1].c_str(), in2[3].c_str(), 1.0 / sqrt(2));
			print("const auto t2 = (%s + %s) * %.17e;\n", in2[1].c_str(), in2[3].c_str(), 1.0 / sqrt(2));
			print("%s = %s + t1\n;", out2[0].c_str(), in2[0].c_str());
			print("%s = -%s - t2\n;", out2[2].c_str(), in2[2].c_str());
			print("%s = %s - t1\n;", out2[1].c_str(), in2[0].c_str());
			print("%s = %s - t2\n;", out2[3].c_str(), in2[2].c_str());
			deindent();
			print("}\n");
		}
		for (int i = 0; i < negs.size(); i++) {
		...
		/*		print("{\n");
		 ...
		 print("%s = y%ir%i + zrs;\n", out[0 + (k + 0 * N / 4)], N, 2 * k + 0 * N / 2);
		 print("%s = y%ir%i;\n", out[0 + (k + 1 * N / 4)], N, 2 * k + 1 * N / 2);
		 print("%s = -zrd;\n", out[N - (k + 1 * N / 4)]);
		 print("%s = y%ir%i - zrs;\n", out[(k + 2 * N / 4)], N, 2 * k + 0 * N / 2);
		 }
		 deindent();
		 print("}\n");
		 ocnt += 6;
		 ...
		 print("%s = y%ir%i + zrs;\n", out[0 + (k + 0 * N / 4)], N, 2 * k + 0 * N / 2);
		 print("%s = y%ii%i + zis;\n", out[N - (k + 0 * N / 4)], N, 2 * k + 0 * N / 2);
		 print("%s = y%ir%i + zid;\n", out[0 + (k + 1 * N / 4)], N, 2 * k + 1 * N / 2);
		 print("%s = y%ii%i - zrd;\n", out[N - (k + 1 * N / 4)], N, 2 * k + 1 * N / 2);
		 print("%s = y%ir%i - zrs;\n", out[N - (k + 2 * N / 4)], N, 2 * k + 0 * N / 2);
		 print("%s = -y%ii%i + zis;\n", out[0 + (k + 2 * N / 4)], N, 2 * k + 0 * N / 2);
		 print("%s = y%ir%i - zid;\n", out[N - (k + 3 * N / 4)], N, 2 * k + 1 * N / 2);
		 print("%s = -y%ii%i - zrd;\n", out[0 + (k + 3 * N / 4)], N, 2 * k + 1 * N / 2);
		 ...
		 if (N > 4) {
		 int k = N / 8;
		 print("const auto t1 = (y%ir%i - y%ir%i) * %.17e;\n", N, 4 * k + 1, N, 4 * k + 3, 1.0 / sqrt(2));
		 print("const auto t2 = (y%ir%i + y%ir%i) * %.17e;\n", N, 4 * k + 1, N, 4 * k + 3, 1.0 / sqrt(2));
		 print("%s = y%ir%i + t1\n;", out[0 + (k + 0 * N / 4)], N, 2 * k + 0 * N / 2);
		 print("%s = y%ir%i - t2\n;", out[N - (k + 0 * N / 4)], N, 2 * k + 1 * N / 2);
		 print("%s = y%ir%i - t1\n;", out[0 + (k + 1 * N / 4)], N, 2 * k + 0 * N / 2);
		 print("%s = -y%ir%i - t2\n;", out[N - (k + 1 * N / 4)], N, 2 * k + 1 * N / 2);
		 ocnt += 6;
		 }
		 */
...
int print_short_fft_prime_factor(int N, std::vector<std::string> in_, std::vector<std::string> out_, bool real = false) {
	int ocnt = 0;
	std::vector<const char*> in(in_.size());
	std::vector<const char*> out(out_.size());
	for (int i = 0; i < in.size(); i++) {
...
int print_short_fft(int N, std::vector<std::string> in_, std::vector<std::string> out_, bool real) {
	int ocnt = 0;
	std::vector<const char*> in(in_.size());
	std::vector<const char*> out(out_.size());
	for (int i = 0; i < in.size(); i++) {
	...
	if (pfac.size() == 1) {
		if (N % 4 == 0) {
			ocnt = print_short_fft_split_radix(N, in_, out_, real);
		} else if (pfac[0].second > 1) {
			ocnt = print_short_fft_common_factor(N, in_, out_, real);
		} else {
			ocnt = print_short_fft_singleton(N, in_, out_, real);
		}
	} else {
		ocnt = print_short_fft_prime_factor(N, in_, out_, real);
	}
