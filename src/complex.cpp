...
void fft_radix_dif(int r, int N, int o) {
	...
	for (int n = 0; n < N / r; n++) {
		...
		indent();
		for (int i = 0; i < r; i++) {
			print("const auto zr%i = x[%i];\n", i, index(o + i * N / r, n, 0, N / r));
			print("const auto zi%i = x[%i];\n", i, index(o + i * N / r, n, 1, N / r));
		}
		switch (r) {
		case 1:
			return;
		...
		default:
			...
			print("const auto qr0 = ", index(o, n, 0, N));
			for (int i = 0; i < r; i++) {
				print_notab(" + zr%i", i);
			}
			print_notab(";\n");
			print("const auto qi0 = ", index(o, n, 1, N));
			for (int i = 0; i < r; i++) {
				print_notab(" + zi%i", i);
			}
			print_notab(";\n");
		...
		for (int q = 0; q < r; q++) {
			...
			print("x[%i] = std::fma((%.17e), qr%i, (%.17e) * qi%i);\n", index(o + q * N / r, n, 0, N), W.real(), q, -W.imag(), q);
			print("x[%i] = std::fma((%.17e), qr%i, (%.17e) * qi%i);\n", index(o + q * N / r, n, 1, N), W.imag(), q, W.real(), q);
		}
